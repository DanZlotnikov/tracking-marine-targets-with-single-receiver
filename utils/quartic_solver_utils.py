...
def ferrari_quartic_solver(a, b, c, d, e, ver='v2'):
    if ver == 'v1':
        return ferrari_quartic_solver_v1(a, b, c, d, e)
    else:
        return ferrari_quartic_solver_v2(a, b, c, d, e)


# based on https://en.wikipedia.org/wiki/Quartic_equation#Ferrari's_solution
def ferrari_quartic_solver_v2(A, B, C, D, E):
    ...
    R = (- Q / 2 + np.sqrt(0j + (Q ** 2) / 4 + (P ** 3) / 27),
         - Q / 2 - np.sqrt(0j + (Q ** 2) / 4 + (P ** 3) / 27))
    R = (np.where(R[0].imag != 0, np.nan, R[0].real), np.where(R[1].imag != 0, np.nan, R[1].real))

    U = np.cbrt(R[1])
    U_tag = np.where(U == 0, np.nan, U)

    y = - (5 / 6) * alpha + (U == 0) * (0 - np.cbrt(Q)) + (U != 0) * (U - np.divide(P, 3 * U_tag))
    W = np.sqrt(0j + alpha + 2 * y)

    x0 = - B / (4 * A) + (0 + W + np.sqrt(0j - (3 * alpha + 2 * y + (2 * beta) / W))) / 2
    x1 = - B / (4 * A) + (0 + W - np.sqrt(0j - (3 * alpha + 2 * y + (2 * beta) / W))) / 2
    x2 = - B / (4 * A) + (0 - W + np.sqrt(0j - (3 * alpha + 2 * y - (2 * beta) / W))) / 2
    x3 = - B / (4 * A) + (0 - W - np.sqrt(0j - (3 * alpha + 2 * y - (2 * beta) / W))) / 2

    return np.array([x0, x1, x2, x3]).T


# based on https://stackoverflow.com/questions/35795663/fastest-way-to-find-the-smallest-positive-real-root-of
# -quartic-polynomial-4-degr
def ferrari_quartic_solver_v1(a, b, c, d, e):
    "resolution of P=ax^4+bx^3+cx^2+dx+e=0"
    "CN all coefficients real."
    ...
    if y0[abs(y1.imag) < abs(y0.imag)].shape[0] > 0:
        if y0[abs(y1.imag) < abs(y0.imag)].shape[0] == 1:
            y0[abs(y1.imag) < abs(y0.imag)][0] = y1[abs(y1.imag) < abs(y0.imag)]
        else:
            y0[abs(y1.imag) < abs(y0.imag)] = y1[abs(y1.imag) < abs(y0.imag)]

    if y0[abs(y2.imag) < abs(y0.imag)].shape[0] > 0:
        if y0[abs(y2.imag) < abs(y0.imag)].shape[0] == 1:
            y0[abs(y2.imag) < abs(y0.imag)][0] = y2[abs(y2.imag) < abs(y0.imag)]
        else:
            y0[abs(y2.imag) < abs(y0.imag)] = y2[abs(y2.imag) < abs(y0.imag)]
    ...
    return np.array([r0 - z0, r1 - z0, r2 - z0, r3 - z0]).T
...
def cardano(a, b, c, d):
    ...
    cond1 = (((w0 < w1) & (w2 < w0)) + 0) * (v * Jc)
    cond2 = (((w0 < w1) & (w0 <= w2)) + 0) * (v)
    cond3 = (((w1 <= w0) & (w2 < w1)) + 0) * (v * Jc)
    cond4 = (((w1 <= w0) & (w1 <= w2)) + 0) * (v * J)

    v = cond1 + cond2 + cond3 + cond4
